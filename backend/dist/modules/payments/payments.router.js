...
paymentsRouter.post("/shop/checkout", requireAuth, async (req, res, next) => {
    try {
        ...
        if (missing)
            throw Object.assign(new Error("Some products not found"), { status: 400 });
    ...
    try {
        ...
        if (tx.ticketId) {
            ...
            if (ticket && ticket.status !== "PAID") {
                const qr = await QRCode.toDataURL(JSON.stringify({ code: ticket.code, ticketId: ticket.id }), { margin: 1, width: 320 });
