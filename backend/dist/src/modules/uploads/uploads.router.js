...
const storage = multer.diskStorage({
    destination: (req, _file, cb) => {
        const raw = String(req.body?.folder || "misc");
...
uploadsRouter.post("/presign", requireAuth, requireRole(["SUPER_ADMIN", "CLUB_ADMIN", "EDITOR"]), async (req, res, next) => {
    try {
        ...
        if (!endpoint)
            throw Object.assign(new Error("S3_ENDPOINT missing"), { status: 500 });
...
uploadsRouter.post("/local", requireAuth, requireRole(["SUPER_ADMIN", "CLUB_ADMIN", "EDITOR"]), upload.single("file"), async (req, res, next) => {
    try {
        const f = req.file;
        if (!f)
