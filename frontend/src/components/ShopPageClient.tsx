...
type TxStatusResp = { status: "PENDING" | "SUCCESS" | "FAILED"; orderId?: string };
...
function loadCart(): CartItem[] {
  ...
  try {
    ...
    // keep only valid variant-based lines (guest checkout requires variantId)
...
export default function ShopPageClient() {
  ...
  // URL params
  const kitParam = (sp.get("kit") || "").toLowerCase(); // home/away/third (from HomeShopSection)
  const categoryParam = (sp.get("category") || "").toUpperCase(); // KIT / MERCH
  const kitTypeParam = (sp.get("kitType") || "").toUpperCase(); // HOME/AWAY/THIRD
  ...
  const [phone, setPhone] = useState(""); // mpesa prompt + receipt
  ...
  const [payState, setPayState] = useState<"IDLE" | "STARTING" | "WAITING" | "SUCCESS" | "FAILED">("IDLE");
  ...
  function inc(variantId: string) {
    persist(cart.map((c) => (c.variantId === variantId ? { ...c, qty: Math.min(20, c.qty + 1) } : c)));
  }
  ...
  function pushUrl(next: { category?: string; kit?: string; kitType?: string; q?: string }) {
    ...
    // prefer kit param for HOME/AWAY/THIRD compatibility with HomeShopSection
    if (next.kit) {
      qs.set("kit", next.kit);
    } else if (next.category) {
      qs.set("category", next.category);
      if (next.category === "KIT" && next.kitType) {
        // also set kit=home style for consistency
  ...
  function addSelectedToCart() {
    ...
    const next = existing
      ? cart.map((c) => (c.variantId === selectedVariant.id ? { ...c, qty: Math.min(20, c.qty + 1) } : c))
      : [
          ...cart,
          {
  ...
  function onAddFromListing(p: ProductListItem) {
    if (p.variantCount === 1 && p.defaultVariant?.id) {
      ...
      const next = existing
        ? cart.map((c) => (c.variantId === p.defaultVariant!.id ? { ...c, qty: Math.min(20, c.qty + 1) } : c))
        : [
            ...cart,
            {
              productId: p.id,
  ...
  async function startStk() {
    ...
    try {
      ...
      pollTimerRef.current = window.setInterval(async () => {
        try {
          const s = await apiJson<TxStatusResp>(`/payments/tx/status/${encodeURIComponent(resp.checkoutRequestId)}`);
    ...
    <div className="pb-24 md:pb-0">
      ...
      <div className="mt-5 flex flex-wrap items-center gap-2">
        <button
          ...
          className={`px-4 py-2 rounded-full border text-xs font-extrabold tracking-widest ${
            !derivedCategory ? "border-brand bg-brand text-black" : "border-line bg-white/5 text-white/80 hover:bg-white/10"
          }`}
        >
          ALL
        </button>
        ...
        <button
          onClick={() => pushUrl({ category: "KIT" })}
          className={`px-4 py-2 rounded-full border text-xs font-extrabold tracking-widest ${
            derivedCategory === "KIT" ? "border-brand bg-brand text-black" : "border-line bg-white/5 text-white/80 hover:bg-white/10"
          }`}
        >
          KIT
        </button>
        ...
        <button
          ...
          className={`px-4 py-2 rounded-full border text-xs font-extrabold tracking-widest ${
            derivedCategory === "MERCH" ? "border-brand bg-brand text-black" : "border-line bg-white/5 text-white/80 hover:bg-white/10"
          }`}
        >
          MERCH
        </button>
      ...
      <div className="mt-4">
        <input
          ...
          onChange={(e) => {
            setQ(e.target.value);
            // keep URL q lightly synced (optional)
            // pushUrl({ q: e.target.value });
          }}
          placeholder="Search products…"
          className="w-full sm:w-[420px] rounded-full border border-line bg-white/5 px-4 py-2 text-sm outline-none focus:ring-2 focus:ring-white/10"
        />
      </div>
      ...
      {loading ? (
        ...
        <div className="mt-6 grid gap-6 grid-cols-2 md:grid-cols-3">
          {products.map((p) => (
            <div key={p.id} className="rounded-3xl border border-line bg-card overflow-hidden">
              ...
              <div className="p-4">
                ...
                <div className="mt-2 font-bold line-clamp-2">{p.title}</div>
                ...
                <button
                  ...
                  className="mt-3 w-full rounded-full bg-brand text-black font-extrabold py-2.5"
                >
                  Add to cart
                </button>
              </div>
      ...
      <div className="hidden md:block mt-10 rounded-3xl border border-line bg-card p-6">
        <CartAndCheckout
          cart={cart}
          inc={inc}
          dec={dec}
      ...
      <button
        onClick={() => setCartOpen(true)}
        className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-ink text-white"
      >
      ...
      {cartOpen && (
        <div className="md:hidden fixed inset-0 z-50">
          ...
          <div className="absolute bottom-0 left-0 right-0 rounded-t-3xl bg-white p-4 max-h-[85vh] overflow-auto">
            <div className="flex items-center justify-between">
              <div className="text-lg font-extrabold text-ink">Checkout</div>
              <button className="text-sm font-bold text-ink" onClick={() => setCartOpen(false)}>
                Close
              </button>
            </div>
      ...
      {pickerOpen && pickerProduct && (
        <div className="fixed inset-0 z-50">
          <div className="absolute inset-0 bg-black/55" onClick={() => setPickerOpen(false)} />

          {/* Desktop modal */}
          <div className="hidden md:block absolute left-1/2 top-1/2 w-[680px] -translate-x-1/2 -translate-y-1/2 rounded-3xl border border-line bg-card p-6">
            <PickerContent
          ...
          {/* Mobile bottom sheet */}
          <div className="md:hidden absolute bottom-0 left-0 right-0 rounded-t-3xl bg-white p-4 max-h-[85vh] overflow-auto">
            <PickerContent
              product={pickerProduct}
              group={pickerGroup}
              setGroup={setPickerGroup}
...
function PickerContent(props: {
  ...
  const { product, group, setGroup, variants, variantId, setVariantId, unitPrice, selectedVariant, onClose, onAdd, mobile } = props;
  ...
  return (
    <div className={mobile ? "text-ink" : ""}>
      <div className="flex items-center justify-between">
        <div className="text-lg font-extrabold">{mobile ? "Select Size" : "Select group & size"}</div>
      ...
      <div className="mt-4 flex gap-4">
        ...
        <div className="min-w-0">
          <div className="font-extrabold line-clamp-2">{product.title}</div>
          <div className="mt-1 text-sm text-muted">Price: <span className="font-extrabold">{priceLabel}</span></div>
          {selectedVariant ? (
            <div className="mt-1 text-xs text-muted">
              Selected: <span className="font-extrabold">{selectedVariant.group === "ADULT" ? "Adults" : "Kids"} · {selectedVariant.size}</span>
            </div>
          ) : null}
        </div>
      </div>
          ...
          <div className="mt-2 inline-flex rounded-full border border-line bg-white/10 p-1">
            {hasAdult && (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-extrabold ${
                  group === "ADULT" ? "bg-brand text-black" : "text-muted hover:text-ink"
                }`}
              >
                Adults
              </button>
            ...
            {hasKids && (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-extrabold ${
                  group === "KIDS" ? "bg-brand text-black" : "text-muted hover:text-ink"
                }`}
              >
                Kids
              </button>
          ...
          {variants.map((v) => {
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-full border text-sm font-extrabold transition ${
                  active ? "border-brand bg-brand text-black" : "border-line bg-white/10 text-ink hover:bg-black/5"
      ...
      <button onClick={onAdd} className="mt-6 w-full rounded-full bg-brand text-black font-extrabold py-3">
        Add to cart
      </button>
    </div>
  );
      ...
      {cart.length === 0 ? (
        ...
        <>
          <div className="space-y-3">
            {cart.map((c) => (
              <div key={c.variantId} className="flex items-center gap-3 rounded-2xl border border-line bg-white/70 p-3">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => dec(c.variantId)} className="h-9 w-9 rounded-full border border-line font-extrabold">
                    −
                  </button>
                  <div className="w-8 text-center font-extrabold">{c.qty}</div>
                  <button onClick={() => inc(c.variantId)} className="h-9 w-9 rounded-full border border-line font-extrabold">
                    +
                  </button>
                </div>

                <button onClick={() => remove(c.variantId)} className="text-sm font-extrabold text-red-600">
                  Remove
                </button>
              </div>
            ))}
          ...
          <div className="mt-5 rounded-2xl border border-line bg-white/70 p-4">
            <div className="flex items-center justify-between">
              ...
              <div className="text-lg font-extrabold">{total} {currency}</div>
            </div>
            ...
            <div className="mt-4 grid gap-3">
              ...
              <Field label="PHONE (M-PESA)" value={phone} onChange={setPhone} placeholder="07xx… or 2547xx…" />
            </div>

            <div className="mt-5">
              <div className="text-xs font-extrabold tracking-widest text-muted">BILLING ADDRESS</div>
              <div className="mt-2 grid gap-3">
                <Field label="ADDRESS LINE 1" value={addressLine1} onChange={setAddressLine1} placeholder="Street / Estate / Plot" />
                <Field label="ADDRESS LINE 2 (OPTIONAL)" value={addressLine2} onChange={setAddressLine2} placeholder="Apartment / Landmark" />
                <Field label="CITY / TOWN" value={city} onChange={setCity} placeholder="Mombasa" />
              </div>
            </div>

            <div className="mt-5">
              <div className="text-xs font-extrabold tracking-widest text-muted">DELIVERY</div>
              <div className="mt-2 grid gap-3">
                <Field label="DELIVERY LOCATION" value={deliveryLocation} onChange={setDeliveryLocation} placeholder="Area / Stage / Landmark" />
                <Field label="DELIVERY NOTES (OPTIONAL)" value={deliveryNotes} onChange={setDeliveryNotes} placeholder="Extra instructions…" />
              </div>
            </div>
            ...
            <button
              ...
              className="mt-4 w-full rounded-full bg-brand text-black font-extrabold py-3 disabled:opacity-60"
            >
              {payState === "STARTING"
                ? "Starting STK…"
                : payState === "WAITING"
            ...
            {payState === "SUCCESS" && (
              <div className="mt-4 rounded-2xl bg-green-50 border border-green-200 p-4">
                <div className="font-extrabold text-green-800">Payment successful ✓</div>
                {orderId ? (
...
function Field(props: { label: string; value: string; onChange: (v: string) => void; placeholder?: string }) {
  return (
    <div>
      <label className="text-[10px] font-extrabold tracking-[0.2em] uppercase text-muted">
        {props.label}
      </label>
      <input
        value={props.value}
