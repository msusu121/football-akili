  ...
  return (
    <div className="hidden sm:flex w-8 h-8 md:w-10 md:h-10 rounded-full bg-gray-100 items-center justify-center flex-shrink-0">
      {logoUrl ? (
        ...
        <img src={logoUrl} alt={name} className="w-6 h-6 md:w-8 md:h-8 object-contain" />
      ) : (
...
function MatchCard({
  ...
  const played = mode === "results" ? true : isResultMatch(match); // fixtures tab always shows time pill
  ...
  return (
    <div className="snap-start w-[300px] sm:w-[320px] bg-white rounded-xl border border-black/10 shadow-[0_10px_24px_rgba(0,0,0,0.08)] overflow-hidden flex-shrink-0">
      ...
      <div className="px-5 pt-5 pb-3 text-center">
        <div className="text-[12px] font-extrabold text-[#2b2b2b] tracking-wide">{league}</div>
        ...
        <div className="flex items-center justify-center gap-3">
          ...
          {played ? (
            <div className="flex items-center gap-2">
              <span className="text-3xl font-extrabold text-[#111]">{match.homeScore ?? "-"}</span>
              <span className="text-[#bbb] font-extrabold text-xl">-</span>
              <span className="text-3xl font-extrabold text-[#111]">{match.awayScore ?? "-"}</span>
            </div>
          ) : (
            <div className="px-4 py-2 bg-[#1a1a2e] rounded-sm">
              <span className="text-white font-extrabold text-[14px] tracking-wide">{fmtTime(dateStr)}</span>
            </div>
          )}
...
export function MatchesRail({
  ...
  const fixtureItems = useMemo(() => (Array.isArray(fixtures) ? fixtures : []).slice(0, maxCards), [fixtures, maxCards]);
  const resultItems = useMemo(() => (Array.isArray(results) ? results : []).slice(0, maxCards), [results, maxCards]);

  const items = tab === "fixtures" ? fixtureItems : resultItems;

  useEffect(() => {
    // reset rail to start when switching tabs (Man Utd feel)
  ...
  <section className="bg-[#f3f3f3] border-t border-black/5">
    {/* Header stays aligned to your normal site width */}
